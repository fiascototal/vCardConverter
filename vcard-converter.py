...
def convert2html(contacts, output_path):
    ...
    for contact in contacts:
        name = _escape(contact['name'])
        tel = ", ".join(contact['tel'])
        adr = _escape(", ".join(contact['adr']))
        ...
        html_content += template_one_entry % (name, photo, tel, email, adr)
...
def _main():
    # manage some options
    u = "%s: [option] <vcard_filepath>" % sys.argv[0]
    d = "Convert a VCard file nto a csv file or an HTML web page"
    parser = optparse.OptionParser(version=VERSION, usage=u, description=d)
